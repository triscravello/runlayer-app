...
function mapRecommendation(item: ScoredRecommendationItem, index: number): RankedGearRecommendation {
  ...
  const primaryReason = item.reasons[0] ?? "This item scored well against the available run, weather, and profile inputs.";
  ...
  return {
    ...
    description: `${brand} ${item.item.name} is ranked for ${category.toLowerCase()} coverage based on live recommendation-engine scoring.`,
    why: item.reasons.length ? item.reasons.join(" ") : primaryReason,
    signals: [
      { label: "Weather fit", value: `Weather contribution: ${item.scoreBreakdown.weather} points` },
      { label: "Run fit", value: `Intensity and terrain contributed ${item.scoreBreakdown.intensity + item.scoreBreakdown.terrain} points` },
      { label: "Profile fit", value: `Profile preferences contributed ${item.scoreBreakdown.brandAffinity + item.scoreBreakdown.budget + item.scoreBreakdown.temperatureTolerance} points` },
    ],
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <section className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-end">
          <div className="space-y-5">
            <Badge variant="outline" className="rounded-full border-emerald-200 bg-white/80 px-3 py-1 text-emerald-700 shadow-sm"><Sparkles className="size-3.5" /> Recommendation result</Badge>
            <div className="space-y-3">
              <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-slate-950 md:text-5xl">Ranked gear for your next run</h1>
              <p className="max-w-2xl text-lg leading-8 text-muted-foreground">These recommendations are generated from your current weather, run context, and saved profile preferences—not static mock data.</p>
