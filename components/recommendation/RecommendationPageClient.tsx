...
function mapRecommendation(item: ScoredRecommendationItem, index: number): RankedGearRecommendation {
  ...
  return {
    ...
    description: `${brand} ${item.item.name} gives you ${category.toLowerCase()} coverage for the run conditions and profile signals availble today.`,
...
export function RecommendationPageClient({ user, profile, engineVersion }: RecommendationPageClientProps) {
  ...
  const alternativeGear = useMemo(() => result?.alternatives?.map(mapRecommendation) ?? gear.filter((item) => !outfitGear.some((outfitItem) => outfitItem.id === item.id)), [gear, outfitGear, result]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-emerald-50/60 via-background to-background px-4 py-8 md:px-8 md:py-12">
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <section className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-end">
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-lg leading-8 text-muted-foreground">Start with a balanced top, bottom, and a accessory when available, then compare non-duplicate alternatives selected for your weather, run context, and saved profile.</p>
        ...
        {!isLoading && !error && alternativeGear.length ? <section className="grid gap-4" aria-label="Alternative gear recommendations"><div><h2 className="text-2xl font-semibold text-slate-950">Alternatives</h2><p className="mt-1 text-muted-foreground">Additional non-duplicate options after the core outfit picks.</p></div>{alternativeGear.map((gearItem) => <Card key={`alternative-${gearItem.id}`} className="border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span>{gearItem.brand}</span><ChevronRight className="size-4" /><span>{gearItem.category}</span></div><h3 className="mt-1 text-xl font-semibold text-slate-950">{gearItem.name}</h3><p className="mt-1 text-sm text-muted-foreground">{gearItem.why}</p></div><Badge className="w-fit rounded-full bg-slate-950 px-3 py-1 text-white">{gearItem.matchLabel}</Badge></div></Card>)}</section> : null}
      </div>
    </main>
  );
}